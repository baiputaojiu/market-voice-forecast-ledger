...
- 現在進行中のアプリ実装タスクはない。
- 承認済み詳細計画に従い、M2本実装と分離したフィージビリティ・スパイクの実行準備を進めている。

## 未着手（Not Started）

### M2事前フィージビリティ検証

1. `.worktrees/` を公開対象から除外し、`superpowers:using-git-worktrees` に従って隔離worktreeを作る。
2. 本番packageを変更せず、使い捨て実験コードと合成データで全scenarioを実行する。
3. 結果、性能値、設計・計画の修正候補を報告し、適用可否の承認を受ける。
