...
- 現在進行中のアプリ実装タスクはない。
- 承認済みスパイク設計を書面化し、書面specのユーザーレビューへ進めている。

## 未着手（Not Started）

### M2事前フィージビリティ検証

1. 書面化したスパイクspecのユーザーレビューを受ける。
2. 小粒度の詳細実行計画を作成し、ユーザー承認を受ける。
3. `.worktrees/` を公開対象から除外し、`superpowers:using-git-worktrees` に従って隔離worktreeを作る。
4. 本番packageを変更せず、使い捨て実験コードと合成データで全scenarioを実行する。
5. 結果、性能値、設計・計画の修正候補を報告し、適用可否の承認を受ける。
