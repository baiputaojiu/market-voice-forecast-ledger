...
M0「複数PC間の作業状態保存・再開基盤」は完了。M1「アプリ設計の完成」を進行中で、中核データモデルと処理状態の設計をユーザーが承認した。正式specのユーザーレビュー後に、このサブプロジェクトの詳細実装計画を作る。アプリ本体は実装未着手。
...
- 現在進行中のアプリ実装作業はない。
- M1中核データモデルspecのユーザーレビュー待ち。レビュー承認前に詳細実装計画や実装へ進まない。
...
1. ユーザーが `docs/superpowers/specs/2026-08-14-core-data-model-design.md` の記録内容をレビューする。
2. レビュー承認後、SQLite schema、migration、repository、監査、job state machineの詳細実装計画を作る。
3. 計画承認後、テスト先行で中核データモデルを実装する。
