...
- スモールテストから得た市場全般表現の推定割当、転換点、条件付き予想、3種類の日付、内部データ削除、段階別進捗の方針を確定要件と決定事項へ反映した。

## 作業中（In Progress）

- 現在進行中のアプリ実装作業はない。
- 次の作業として、話者割当、本人発言抽出、Codex分析、対象指数割当を別データにするM1のデータモデルと変更不能スナップショット形式の設計を開始できる状態。
...
- 参照声照合の信頼度尺度・閾値、保留レビュー、収録日の取得・手入力、全文文字起こしの保存期限と削除仕様の確定。
- M2以降のアプリ実装。
...
- 収録日を自動取得できない動画の入力・確認方法。
- 全文文字起こしの既定保存期限、ユーザー指定範囲、削除後に残す監査メタデータ。
- 自動指数割当の信頼度尺度と、ヒートマップ採用可否の閾値。

## 次の作業（Next Actions）

1. M1のデータモデルを設計し、動画、発話区間、話者割当、本人発言抽出、分析run、自動指数割当、予想、チェックポイントを別エンティティにする。
2. 変更不能スナップショット、5～10分チェックポイント、段階別の実完了数による進捗manifestを定義する。
3. 本人発言だけを入力し、外部ツール0件を監査できるCodex promptとJSON Schemaを確定する。
4. 市場全般表現の自動割当、転換点、条件付き、期間正規化、空欄と判断不能、ヒートマップ採用可否の集約規則を確定する。
5. 参照声照合、収録日フォールバック、内部データの保存期限・削除、UI表示の未解決事項を決める。
